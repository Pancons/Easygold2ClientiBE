...
namespace EasyGold.API.Controllers
{
    ...
    {
        private readonly ITagProdottiService _service;

        public TagProdottiController(ITagProdottiService service)
        {
            _service = service;
        }
        ...
        [HttpGet("{id}")]
        [Authorize]
        [ProducesResponseType(typeof(TagProdottiDTO), StatusCodes.Status200OK)]
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> Save([FromBody] TagProdottiDTO dto)
        {
            try
            {
                if (dto == null)
                ...
                if (dto.Etp_IDAuto.HasValue && dto.Etp_IDAuto > 0)
                {
