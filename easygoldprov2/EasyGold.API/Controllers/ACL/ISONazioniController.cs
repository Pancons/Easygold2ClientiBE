...
namespace EasyGold.API.Controllers.ACL
{
    ...
    public class ISONazioniController : ControllerBase
    {
        ...
        private readonly string _baseUri = "http://sistemaeasygold.ddns.net/api/ISONazioni"; // Assicurati che l'URL sia corretto

        public ISONazioniController(HttpClient httpClient, ISistemaEasyGoldTokenService tokenService)
        {
        ...
        public async Task<IActionResult> GetISONazioniList([FromBody] ISONazioniListRequest filter)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var result = JsonSerializer.Deserialize<BaseListResponse<ISONazioniDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        [HttpPost("save")]
        [Authorize]
        [ProducesResponseType(typeof(ISONazioniDTO), StatusCodes.Status200OK)]
        ...
        public async Task<IActionResult> AddOrUpdateISONazione([FromBody] ISONazioniDTO dto)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<ISONazioniDTO>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        public async Task<IActionResult> GetISONazione(int id)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<BaseResponse<ISONazioniDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
