...
namespace EasyGold.API.Controllers.ACL
{
    ...
    public class CauProgressiviController : ControllerBase
    {
        ...
        private readonly string _baseUri = "http://sistemaeasygold.ddns.net/api/cauProgressivi"; // Assicurati che l'URL sia corretto

        public CauProgressiviController(HttpClient httpClient, ISistemaEasyGoldTokenService tokenService)
        {
        ...
        public async Task<IActionResult> GetCauProgressiviList([FromBody] CauProgressiviListRequest filter)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var result = JsonSerializer.Deserialize<BaseListResponse<CauProgressiviDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        [HttpPost("save")]
        [Authorize]
        [ProducesResponseType(typeof(CauProgressiviDTO), StatusCodes.Status200OK)]
        ...
        public async Task<IActionResult> AddOrUpdateCauProgressivo([FromBody] CauProgressiviDTO dto)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<CauProgressiviDTO>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        public async Task<IActionResult> GetCauProgressivo(int id)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<BaseResponse<CauProgressiviDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
