...
namespace EasyGold.API.Controllers.ACL
{
    ...
    public class IdiomiEasyGoldController : ControllerBase
    {
        ...
        private readonly string _baseUri = "http://sistemaeasygold.ddns.net/api/idiomiEasygold"; // Assicurati che l'URL base sia corretto

        public IdiomiEasyGoldController(HttpClient httpClient, ISistemaEasyGoldTokenService tokenService)
        {
        ...
        public async Task<IActionResult> GetIdiomiList([FromBody] IdiomiEasyGoldListRequest filter)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var result = JsonSerializer.Deserialize<BaseListResponse<IdiomiEasyGoldDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        [HttpPost("save")]
        [Authorize]
        [ProducesResponseType(typeof(IdiomiEasyGoldDTO), StatusCodes.Status200OK)]
        ...
        public async Task<IActionResult> AddOrUpdateIdioma([FromBody] IdiomiEasyGoldDTO dto)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<IdiomiEasyGoldDTO>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        public async Task<IActionResult> GetIdioma(int id)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<BaseResponse<IdiomiEasyGoldDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
