...
namespace EasyGold.API.Controllers.ACL
{
    ...
    public class TipoTagProdottiController : ControllerBase
    {
        ...
        private readonly string _baseUri = "http://sistemaeasygold.ddns.net/api/tipoTagProdotti"; // Assicurati che l'URL sia corretto

        public TipoTagProdottiController(HttpClient httpClient, ISistemaEasyGoldTokenService tokenService)
        {
        ...
        public async Task<IActionResult> GetTipoTagProdottiList([FromBody] TipoTagProdottiListRequest filter)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var result = JsonSerializer.Deserialize<BaseListResponse<TipoTagProdottiDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        [HttpPost("save")]
        [Authorize]
        [ProducesResponseType(typeof(TipoTagProdottiDTO), StatusCodes.Status200OK)]
        ...
        public async Task<IActionResult> AddOrUpdateTipoTagProdotto([FromBody] TipoTagProdottiDTO dto)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<TipoTagProdottiDTO>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        public async Task<IActionResult> GetTipoTagProdotto(int id)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<BaseResponse<TipoTagProdottiDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
