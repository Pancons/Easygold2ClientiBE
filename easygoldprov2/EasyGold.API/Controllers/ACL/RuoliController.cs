...
using EasyGold.Web2.Models.Comune.Ruoli;
...
namespace EasyGold.API.Controllers.ACL
{
    ...
    public class RuoliController : ControllerBase
    {
        ...
        private readonly string _baseUri = "http://sistemaeasygold.ddns.net/api/ruoli"; // Assicurati che l'URL base sia corretto

        public RuoliController(HttpClient httpClient, ISistemaEasyGoldTokenService tokenService)
        {
        ...
        public async Task<IActionResult> GetRoles()
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var result = JsonSerializer.Deserialize<BaseListResponse<RuoloDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
