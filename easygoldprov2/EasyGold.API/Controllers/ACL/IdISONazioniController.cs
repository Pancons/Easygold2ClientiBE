...
namespace EasyGold.API.Controllers.ACL
{
    ...
    public class IdISONazioniController : ControllerBase
    {
        ...
        private readonly string _baseUri = "http://sistemaeasygold.ddns.net/api/id_ISONazioni"; // Assicurati che l'URL base sia corretto

        public IdISONazioniController(HttpClient httpClient, ISistemaEasyGoldTokenService tokenService)
        {
        ...
        public async Task<IActionResult> GetIdISONazioniList([FromBody] IdISONazioniListRequest filter)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var result = JsonSerializer.Deserialize<BaseListResponse<IdISONazioniDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        [HttpPost("save")]
        [Authorize]
        [ProducesResponseType(typeof(IdISONazioniDTO), StatusCodes.Status200OK)]
        ...
        public async Task<IActionResult> AddOrUpdateIdISONazione([FromBody] IdISONazioniDTO dto)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<IdISONazioniDTO>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
            else
            {
        ...
        public async Task<IActionResult> GetIdISONazione(int id)
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync();
                var result = JsonSerializer.Deserialize<BaseResponse<IdISONazioniDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
                return Ok(result);
            }
